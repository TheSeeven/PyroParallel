...
TEST_FP32_A = [np.full(1000, 1.1648, dtype=np.float32) for _ in range(100)]
TEST_FP32_B = [np.full(1000, 2.2569, dtype=np.float32) for _ in range(100)]

TEST_FP64_A = [np.full(1000, 1.1648, dtype=np.float64) for _ in range(100)]
TEST_FP64_B = [np.full(1000, 2.2569, dtype=np.float64) for _ in range(100)]
input_image = np.array(Image.open("./sample_data/5120x2880_1_NO_HDR.bmp"))
EDGE_DETECTION_SAMPLE_IMAGE = Framework_all.grayscale(
    [input_image for x in range(100)])
### TEST DATA
